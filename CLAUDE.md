...
**Fixed in April 2026.** The script used to duplicate the lang-switch and hreflang blocks on every run, requiring manual cleanup. That is no longer the case. The generator now strips all existing hreflang and lang-switch blocks before inserting a single canonical copy on both the locale path and the English update path. Running `python3 generate-i18n-pages.py` is now fully idempotent.
...
- **`generate-i18n-pages.py` is idempotent as of April 2026.** Earlier versions required manual dedup of lang-switch and hreflang after each run. Fixed by adding strip-before-insert logic in both locale and English paths.
